...
const MyOrders = () => {
  ...
  useEffect(() => {
    fetch(`${backendURL}/orders/${user.email}`)
      .then((res) => res.json())
      .then((data) => {
        setMyOrders(data);
      });
      ...
      <div>
        <table className="order-table">
          ...
          <tbody>
            {myOrders.map((order) => (
              <tr key={order._id}>
                <td data-title="Product Name">{order.product.name}</td>
                <td data-title="Cost">${order.product.price}</td>
                <td data-title="Ordered by">{order.name}</td>
                <td data-title="Phone">{order.phone}</td>
                <td data-title="Status">{order.shippingStatus}</td>
